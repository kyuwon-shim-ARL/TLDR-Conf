# Community 134 (Level 4.0)

**Size**: 1 papers, 1 unique entities
**Resolution**: 4.0
**Dominant Category**: systems_biology_tools
**Years**: 2017 - 2017

---

**Research Focus**: This community focuses on systems_biology_tools research, primarily utilizing methods such as Comparing.

**Entity Type Distribution**:
- METHOD: 1 unique entities

**Most Frequent Entities**:
- Comparing (METHOD): 2 mentions

**Sample Papers** (showing up to 10):
- (2017) Comparing individual-based approaches to modelling the self-organization of multicellular tissues