# Community 135 (Level 2.0)

**Size**: 1 papers, 1 unique entities
**Resolution**: 2.0
**Dominant Category**: virtual_cell_specific
**Years**: 2021 - 2021

---

**Research Focus**: This community focuses on virtual_cell_specific research, primarily utilizing methods such as Hematopoietic.

**Entity Type Distribution**:
- METHOD: 1 unique entities

**Most Frequent Entities**:
- Hematopoietic (METHOD): 2 mentions

**Sample Papers** (showing up to 10):
- (2021) Hematopoietic mosaic chromosomal alterations increase the risk for diverse types of infection