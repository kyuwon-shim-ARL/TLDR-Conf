# Community 136 (Level 2.0)

**Size**: 1 papers, 1 unique entities
**Resolution**: 2.0
**Dominant Category**: virtual_cell_specific
**Years**: 2022 - 2022

---

**Research Focus**: This community focuses on virtual_cell_specific research, primarily utilizing methods such as WhichTF.

**Entity Type Distribution**:
- METHOD: 1 unique entities

**Most Frequent Entities**:
- WhichTF (METHOD): 2 mentions

**Sample Papers** (showing up to 10):
- (2022) WhichTF is functionally important in your open chromatin data?